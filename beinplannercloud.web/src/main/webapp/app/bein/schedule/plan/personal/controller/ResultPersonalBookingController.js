ptBossApp.controller('ResultPersonalBookingController', function($scope,$translate,parameterService,$location,homerService,commonService,globals) {
	...
	* PAGES
	...
	$scope.selectedProgramPersonal=new Object();
   ...
   $scope.attendies=new Array();
	...
	$scope.initResCB=function(){
		...
		$scope.selectedProgramPersonal.progId=$scope.scheduleObj.schedulePlan.progId;
		$scope.selectedProgramPersonal.progType=$scope.scheduleObj.schedulePlan.progType;
	...
	function getProgramPersonal(progId){
		$.ajax({
			  type:'POST',
			  ...
			  cache:false
				...
				$scope.selectedProgramPersonal=res;
				$scope.$apply();
	...
	$scope.deletePersonals=function(){
		swal({
      		...
      		$.ajax({
      			  ...
      			  cache:false
      				...
      				if(res.resultStatu==1){
      					...
      					$(location).attr("href", "/beincloud/bein/#/schedule/deleteAll" );
	...
	function getTimePlan(){
		$.ajax({
			  type:'POST',
			  url: "../pt/schedule/getSchedule/"+globals.PROGRAM_PERSONAL+"/"+$scope.schedulePlan.schId,
			  contentType: "application/json; charset=utf-8",				    
			  dataType: 'json', 
			  cache:false
			}).done(function(res) {
	...
	$scope.continuePersonal=function(){
