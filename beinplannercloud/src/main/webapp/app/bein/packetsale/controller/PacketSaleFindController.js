	...
	$scope.init = function(){
		...
		if($rootScope.uniquePacket==1){
			if($rootScope.whichPacket==1){
				$(location).attr("href","/beincloud/bein/#/packetsale/personal/list");
			}else if($rootScope.whichPacket==2){
				$(location).attr("href","/beincloud/bein/#/packetsale/class/list");
			}else if($rootScope.whichPacket==3){
				$(location).attr("href","/beincloud/bein/#/packetsale/membership/list");
			}
		}
